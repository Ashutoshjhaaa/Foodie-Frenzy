...
const SpecialOffer = () => {
    ...
    const addButtonBase = "relative flex items-center justify-center gap-2 px-6 py-3 bg-amber-600 rounded-xl font-bold uppercase tracking-widest overflow-hidden group/btn";
    const addButtonHover = "hover:bg-amber-500 hover:scale-105 active:scale-95 shadow-lg shadow-amber-900/20";
    const commonTransition = "transition-all duration-300 ease-out";

    return (
        <div className='bg-gradient-to-b from-[#1a1212] to-[#2a1e1e] text-white py-16 px-4 font-[poppins]'>
            <div className='max-w-7xl mx-auto'>
                <div className='text-center mb-14'>
                    <h1 className='text-5xl font-bold mb-4 transform transition-all bg-gradient-to-r from-amber-400 text-red-500 bg-clip-text text-transparent font-[Playfair_Display] italic'>
                    ...
                    <p className='text-lg text-gray-300 max-w-3xl mx-auto tracking-wide leading-relaxed'>
                        Savor the extraordinary with our exclusive daily specials.
                    </p>
                </div>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8'>
                    {(showAll ? initialData : initialData.slice(0, 4)).map((item, index) => {
                        ...
                        return (
                            <div key={`${item.id}-${index}`} className='relative group bg-[#4b3b3b] rounded-3xl overflow-hidden shadow-2xl transform hover:-translate-y-4 transition-all duration-500 hover:shadow-red-900/40 border-2 border-transparent hover:border-amber-500/20'>
                                    ...
                                    <div className='flex justify-between items-center  gap-4'>
                                        ...
                                        {existingItem ? (
                                            <div className='flex items-center gap-3'>
                                                <button onClick={() => {existingItem.quantity > 1 ? updateQuantity(item, existingItem.quantity -1)
                                                : removeFromCart(item)
                                                ...
                                                <span className='w-8 text-center text-amber-100 font-cinzel'>
                                                 ...
                                                 <button onClick={() => updateQuantity(item, existingItem.quantity +1)}
                                                 className='w-8 h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 transition-all duration-200 active:scale-95'>
                                                    <HiPlus className='w-4 h-4 text-amber-100'/>
                                                </button>
                                                </div>
                                        ):(
                                            <button onClick={() => addToCart({...item, name: item.title, price: item.price }, 1)}
            ...
            <div className='mt-12 flex justify-center'>
                <button onClick={() => setShowAll(!showAll)} className='flex items-center gap-3 bg-gradient-to-r from-red-700 text-white px-8 py-4 rounded-2xl font-bold text-lg uppercase tracking-wider hover:gap-4 hover:scale-105 hover:shadow-amber-500/20 transition-all duration-300 group border-2 border-amber-400/30 relative overflow-hidden'>
