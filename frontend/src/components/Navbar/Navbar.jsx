...
const Navbar = () => {
    ...
    // COMBINE UPDATING LOGIN AND STATUS ON LOCATION CHANGE
    ...
    useEffect(() => {
        ...
        setAuthenticated(Boolean(localStorage.getItem('loginData')));
    }, [location.pathname])

    const hamdleLoginSuccess = () => {
    ...
    const renderDesktopAuthButton = () => {
        return isAuthenticated ? (
            <button onClick={handleLogout} className='px-3 lg:px-4 py-1.5 lg:py-2 bg-gradient-to-br from-amber-500 to-amber-700 text-[#2D1B0E] rounded-2xl font-bold hover:shadow-lg hover:shadow-amber-600/40 transition-all transform hover:scale-[1.02] border-amber-600/20 flex items-center space-x-2  shadow-md shadow-amber-900/20 text-sm'>
        ...
        <nav className='bg-[#2D1B0E] border-b-8 border-amber-900/30 shadow-amber-900/30 sticky top-0 z-50 -shadow-[0_25px_50px_-12px] font-vibes group/nav overflow-x-hidden'>
                    ...
                    <div className='hidden md:flex items-center space-x-2 md:space-x-1 lg:space-x-4 flex-1 justify-end'>
                        ...
                        <div className='flex items-center space-x-2 md:space-x-3 lg:space-x-4 ml-3 md:ml-3 lg:ml-6 mr-2 md:mr-3 lg:mr-4'>
                            ...
                            <NavLink to='/cart' className='p-2 md:p-2.0 lg:p-3 text-amber-100 rounded-xl transition-all relative border-2 border-amber-900/30 hover:border-amber-600/50 group hover:bg-amber-900/20 hover:shadow-lg hover:shadow-amber-500/30 shadow-md shadow-amber-900/20'>
                                ...
                                {totalItems > 0 && (
                                    <span className='absolute -top-2 -right-2 bg-amber-600 text-amber-100 text-xs w-5 h-5 rounded-full flex items-center justify-center font-sans font-bold shadow-sm'>
                                        {totalItems}
                                    </span>
                                )}
                            </NavLink>
                ...
                <div className='md:hidden bg-[#2D1B0e] border-t-4 border-amber-900/40 relative shadow-lg shadow-amber-900/30 w-full'>
                    <div className='px-4 py-4 space-y-2'>
                        {navLinks.map((link) => (
                            <NavLink
                                key={link.name}
                                to={link.to} onClick={() => setIsOpen(false)} className={({ isActive }) => `block px-4 py-3 rounded-xl transition-all items-center ${isActive ? 'bg-amber-600/30 text-amber-400' : 'text-amber-100 hover:bg-amber-600/20'} border-2 ${isActive ? 'border-amber-600/50' : 'border-amber-900/30'}`}>
                        ...
                        <div className='pt-4 border-t-2 border-amber-900/30 space-y-2'>
                            <NavLink to='/cart' onClick={() => setIsOpen(false)}
                                ...
                                {totalItems !== 0 && totalItems !== "0" && (
                                    <span className='absolute -top-1 right-2 bg-amber-600 text-amber-100 text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold'>
                                        {totalItems}
                                    </span>
                                )}
                                <span>Cart</span>
            ...
            {showLoginModal && (
                <div className='fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4'>
                    <div className='bg-gradient-to-br from-[#2D1B0E] to-[#4a372a] rounded-xl p-6 w-full max-w-[480px] relative border-amber-700/30 shadow-[0_0_30px] shadow-amber-500/30'>
                        <button onClick={() => navigate('/')}
                            className='absolute top-2 right text-amber-500 hover:text-amber-300 text-2xl'>
                            &times;
                        </button>
                        ...
                        <Login onLoginSuccess={hamdleLoginSuccess} onClose={() => navigate('/')} />
                    </div>
                </div>
            )}
